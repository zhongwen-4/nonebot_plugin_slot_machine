...
from nonebot import get_driver, load_plugin, logger, on_command, require
...
@slot_rule.handle()
async def handle_slot_rule() -> None:
    await slot_rule.finish(
        "老虎机规则\n"
        "1. 注册：发送“注册老虎机”或“注册”，每人赠送 100 金币。\n"
        "2. 投注：发送“设置投注 <投注大小> <投注倍数>”。\n"
        "   投注大小：0.02、0.2、1；投注倍数：1-10。\n"
        "   投注总额 = 投注大小 x 投注倍数 x 10。\n"
        "3. 抽奖：发送“开始旋转”。盘面为 5 行 6 列。\n"
        "4. 中奖：必须从第一列开始，连续至少 3 列都有同一符号才中奖。\n"
        "   W 是百搭符号，可替代普通符号；S 是夺宝符号。\n"
        "5. 连消：中奖符号会消除并补充新符号，奖金倍数从 x1 开始，"
        "每次中奖后翻倍，最高 x1024。\n"
        "6. 夺宝：盘面任意位置出现 3 个 S 获得 10 次免费抽奖，"
        "超过 3 个时每多 1 个增加 10 次。\n"
        "7. 赔付点数：A=50/100/150/170，B=30/60/100/140，"
        "C、D=20/40/80/100，E、F=10/25/60/80，"
        "G、H=8/15/30/50。\n"
